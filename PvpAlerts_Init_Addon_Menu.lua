...
function PVP:InitializeAddonMenu()
	...
	local optionsData = {
		...
		tooltip = "ON - frame can me moved on the screen by left clicking and dragging, OFF - frame is locked in place and can not be moved",
		...
		setFunc = function(newValue) self.SV.enabled = newValue self:InitControls() self:OnOff() end,
	    },
	    {
		type = "checkbox",
		...
		tooltip = "ON - enables the full system (window, ability to add/remove KOS/COOL players, sounds), OFF - disables the KOS/COOL system",
		...
		tooltip = "ON - notify about hard-hitting channeled abilities, OFF - do not notify about hard-hitting channeled abilities",
		...
		setFunc = function(newValue) self.SV.feedControlScale = newValue/100 self:InitControls() end,
	    },
	    {
		type = "dropdown",
		...
		setFunc = function(newValue) self.SV.newAttackerFrameScale = newValue/100 self:InitControls() end,
	    },
	    {
		type = "slider",
		...
		setFunc = function(newValue) self.SV.KOSControlScale = newValue/100 self:InitControls() end,
	    },
	    {
		type = "checkbox",
		...
		setFunc = function(newValue) self.SV.targetNameFrameScale = newValue/100 self:InitControls() end,
	    },
	    {
		type = "dropdown",
		...
		setFunc = function(newValue) self.SV.targetNameFrameAlpha = newValue/100 self:InitControls() end,
	    },
	    {
		type = "slider",
		name = "Set Target Name Frame fadeout time (ms)",
		tooltip = "Target Name Frame fadeout time goes from 0 (fadeout disabled) to 5000ms (5 sec). Default value is 1500ms (1.5 sec).",
		...
		tooltip = "ON - shows on screen info from for objective, OFF - doesn't show on screen info from for objective",
		...
		tooltip = "ON - icon of the objective will be hidden, while having on screen info, OFF - both on screen info and 3d icon will be present",
		...
		tooltip = "ON - shows group leader 3d icon at any distance, OFF - hides group leader 3d icon when close to player",
		...
		tooltip = "ON - when you put a waypoint marker on the map the addon puts ping icon on top of it, OFF - disables setting ping on waypoint marker",
		...
		tooltip = "Maximum distance goes from 2000m (default, approximately the distance between 2 keeps) down to 250m",
		...
		tooltip = "Maximum distance to show POI icons (fights, delves, wells, etc). It goes from 250m to 1000m (dafault value - 750m)",
		...
		tooltip = "Sets opacity for the district icons outside of your current district. Opacity goes from 0% (invisible) to 100% (fully visible)",
		...
		setFunc = function(newValue) self.SV.neighboringDistrictsAlpha = newValue/100 self:InitControls() end,
	    },
	    {
		type = "checkbox",
		...
		setFunc = function(newValue) self.SV.compass3dHeight = newValue self:InitControls() end,
	    },
	    {
		type = "slider",
		...
		setFunc = function(newValue) self.SV.compass3dSize = newValue self:InitControls() end,
	    },
	    {
		type = "colorpicker",
		...
		setFunc = function(newValue) self.SV.reportSavedInfo = newValue
		...
		tooltip = "ON - the button works as a toggle, OFF - shows scoreboard on button press and hides on button release",
		...
		tooltip = "ON - replaces the default rank 50 icon (5 stars) with a new 6 stars icon, OFF - reverts to 5 star icon for rank 50",
		default = self.defaults.show6star,
		disabled = function() return not self.SV.enabled end,
		getFunc = function() return self.SV.show6star end,
		setFunc = function(newValue) self.SV.show6star = newValue
